...
[![Live Demo](https://img.shields.io/badge/demo-live-brightgreen.svg)](https://quanttrade-8djs.onrender.com/)

**[Live demo →](https://quanttrade-8djs.onrender.com/)** — there's no seeded account, so register a free one from the login screen. It's hosted on Render's free tier, so the first request after a period of inactivity can take ~30-60s to wake the service up.
...
```bash
cd BackEnd && uv run pytest tests/ -v       # 220 tests
cd FrontEnd && npm run test                 # 28 tests
```
...
- Backend: http://localhost:8000 (interactive API docs auto-generated by FastAPI at `/docs`, or `/redoc`)
